...
function Stat({ label, value }) {
  return (
    <div className="card p-4">
      ...
      <p className="font-display font-bold text-2xl md:text-3xl">{value}</p>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6">
        ...
        <Stat label="Rating" value={stats?.avgRating ? stats.avgRating.toFixed(1) : '—'} />
      </div>

      <div className="card p-4 md:p-5">
        <h2 className="font-semibold text-sm mb-4">Últimas calificaciones</h2>
        {recent.length === 0 ? (
            ...
            {recent.map(r => (
              <div key={r.id} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mb-0.5 truncate">{r.course?.title}</p>
